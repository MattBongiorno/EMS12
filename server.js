                ...
                try {
                    const newManager = await inquirer.prompt([{
                        ...
                        connection.query("INSERT INTO employee (??) VALUES (?, ?, ?, ?);", [
                            ["first_name", "last_name", "role_id", "manager_id"], newEmployee[0], newEmployee[1], newEmployee[2], newEmployee[3]
    ...
    connection.query("SELECT * FROM department;", function(err, res) {
        ...
        inquirer.prompt([{
            ...
            connection.query("INSERT INTO role (??) VALUES (?, ?, ?);", [
                ["title", "salary", "department_id"], answers.newRole, answers.newSalary, answers.deptRole
            ], function(err, res) {
                if (err) throw err;
